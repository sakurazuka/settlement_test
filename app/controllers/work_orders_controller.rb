class WorkOrdersController < ApplicationController
  ...
  def index
    ...
    # @work_orders = WorkOrder.search(settlement_work_order_id_null: true).result
    # @work_orders = WorkOrder.search(settlement_work_order_id_null: false).result
    # binding.pry
    q = params[:q] ? params[:q] : {}
    case params[:settlement].to_i
    when 0; ;
    when 1; q[:settlement_work_order_id_null] = false
    when 2; q[:settlement_work_order_id_null] = true
    end
    @q = WorkOrder.search(q)
